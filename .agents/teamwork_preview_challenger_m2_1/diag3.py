...
sim = BinarySimulator()
res = sim.run(df, signals, expiry_candles=1, initial_capital=1e15, bet_fraction=1e-10)
sim_trades = {t['index']: t for t in res['trades']}

labels = create_labels(df, signals, expiry_candles=1)

out_path = r'c:\Users\juanc\Desktop\prueba\.agents\teamwork_preview_challenger_m2_1\diag3.txt'
with open(out_path, 'w', encoding='utf-8') as f:
    ...
    for idx in labels.index:
        l_val = labels.loc[idx]
        if idx not in sim_trades:
            f.write(f"Missing in sim: idx {idx}\n")
            mismatches += 1
        else:
            s_res = sim_trades[idx]['result']
            e_val = 1.0 if s_res == 'WIN' else 0.0
            if l_val != e_val:
                f.write(f"Mismatch at idx {idx}: label={l_val}, sim={s_res}\n")
                mismatches += 1
